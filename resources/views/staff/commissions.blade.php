            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 text-sm">
                <div class="flex items-center justify-between">
                    ...
                    <div class="text-right">
                        ...
                        <div class="text-gray-600 mt-2">Estimated commission</div>
                ...
                <table class="min-w-full divide-y divide-gray-200 text-sm">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse(($payments ?? []) as $p)
                            <?php $comm = \App\Models\Commission::where('payment_id', $p->id)->first(); $rowRate = $comm->rate ?? $rate; $rowComm = $comm->amount ?? round(((float)$p->amount) * $rowRate / 100, 2); ?>
                            <tr>
                                ...
                                <td class="px-6 py-3">{{ number_format($rowComm, 2) }} <span class="text-xs text-gray-500">({{ number_format($rowRate, 2) }}%)</span></td>
                            </tr>
                        @empty
                            <tr>
